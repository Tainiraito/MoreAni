...
describe('parseReviewMarkup', () => {
  ...
  it('round-trips nested formats, line breaks, empty formats, and literal HTML safely', () => {
    ...
    expect(editor.querySelector('[data-review-format="bold"]')).toHaveTextContent('重点 剧透')
    expect(editor.querySelector('[data-review-format="inline-spoiler"]')).toHaveTextContent('剧透')
    expect(editor.querySelector('[data-review-format="bold"]')).toHaveAttribute('data-review-token', '**')
    expect(editor.querySelector('[data-review-format="inline-spoiler"]')).toHaveAttribute('data-review-token', '||')
    const emptyFormat = editor.querySelector('[data-review-format="inline-spoiler"][data-review-empty="true"]')
