...
describe('AppHeader 统计分析入口', () => {
  ...
  it('登录后在通知中心左侧显示统计分析入口', () => {
    ...
    const battle = view.getByRole('link', { name: '打开红蓝合战' })
    ...
    expect(battle).toHaveAttribute('href', '/ratings/battle')
    expect(battle.compareDocumentPosition(analytics) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(analytics.compareDocumentPosition(notification) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    const tooltipTexts = Array.from(view.container.querySelectorAll('[role="tooltip"]')).map(element => element.textContent)
    expect(tooltipTexts).toEqual(expect.arrayContaining(['红蓝合战', '偏好分析', '通知中心']))
