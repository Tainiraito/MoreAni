...
- **本地提交**: 可以自动执行
- **推送到远程**: 必须先获得用户确认
- **删除远程分支**: 必须先确认分支已合并且获得用户批准
- 未经用户明确批准，不得 `git push`

### 任务分析与分支隔离（继承全局规范）

- 所有会写入仓库的功能、Bug、重构、测试、文档和配置任务，修改前必须先完成只读分析：阅读适用的 `AGENTS.md`、执行 `git status --short --branch`、确认影响范围、测试与验收标准。
- 一个任务使用一个主题分支；同一功能的实现、测试、类型、文档和元数据可以放在同一分支，能够独立合并、回滚或验收的内容必须拆分。
- 当前工作区有未提交改动时，先列出改动并确认归属；不得自动 stash、提交、切换、搬运、覆盖或重置用户改动。
- 除非用户当前明确要求直接修改当前分支，所有写入任务都应在修改前建立独立分支，并先说明基线、分支名和修改范围。
- 小型任务可以简化分析说明，但不能跳过状态检查和分支隔离。

### 本地开发服务与直接测试

- 修改运行时代码、API、前端、配置或测试相关行为后，完成任务前必须确保后端和/或前端本地开发服务已启动，并确认服务加载了当前修改。
- 后端开发服务：`cd backend && source venv/bin/activate && uvicorn main:app --reload --host 0.0.0.0 --port 8080`。
- 前端开发服务：`cd frontend && npm run dev`，默认访问 `http://localhost:5173`；前端 API 通过 Vite 代理访问后端 `http://localhost:8080`。
- 后端修改至少验证健康检查或相关 API；前端修改至少访问目标页面并检查运行时错误；前后端联动修改必须同时验证两端。
- 交付时保留相关本地开发服务运行，并报告访问地址、端口和验证结果，方便直接测试；启动失败、端口冲突或代码未加载时必须报告具体错误。
- 纯文档或规范修改可以不启动业务服务，但必须重新读取文件并完成文档校验。
...
- 任务分支统一从最新的 `origin/dev` 创建，PR 默认合并回 `dev`；`main` 仅用于稳定版本发布和通过 PR 合并。
- `dev` 是唯一开发集成基线。每次将 `dev` 合并到 `main` 后，应立即将 `dev` 快进同步到 `main`，避免两个主分支产生漂移。
- 若发生直接合入 `main` 的热修复，完成后必须将 `main` 的变更同步回 `dev`，再创建后续任务分支。
- 主题分支合并并确认不再使用后，应删除对应的本地和远程分支，保留 `main`、`dev` 等长期分支。
- 分支名使用小写英文和短横线：`<type>/<slug>`，不使用代理名称或日期前缀。
- 分支类型：`feat` 新功能、`fix` Bug、`refactor` 重构、`docs` 文档、`test` 测试、`chore` 构建/依赖/格式/维护。
- 仅格式化或样式调整使用 `chore/style-<slug>`；紧急修复使用 `fix/urgent-<slug>`。
...
| 色名 | 色值 | 用途 |
|------|------|------|
| 品牌粉 | `#FB71A7` | 主按钮、强调色、评分星星、Tab 选中 |
| 品牌粉浅 | `rgba(251,113,167,0.15)` | 浅色背景、选中态 |
| 品牌粉深 | `#E85D93` | hover、active 状态 |
...
所有品牌色统一使用 icon 的粉色 `#FB71A7`：

| 色名 | 色值 | 用途 |
|------|------|------|
| 品牌粉 | `#FB71A7` | 主色，按钮、强调色 |
| 品牌粉浅 | `#FECDDA` | 浅色背景 |
| 品牌粉深 | `#E85D93` | hover、渐变终点 |
