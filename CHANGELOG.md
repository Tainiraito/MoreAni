...
### UI/UX 优化
- 修复 `--brand` 变量缺失：头像/评分星星/分数等 10 处 `var(--brand)` 之前全部失效（浅色模式头像白字看不见、设置弹窗头像不显示），:root 补齐变量定义
- 番剧列表双视图：评论列表视图（默认，每行左侧番剧信息 + 右侧朋友评分评论集合，后端列表接口批量返回 recent_reviews）+ 卡片网格视图，筛选区切换按钮，localStorage 记忆选择
- Tab 分类文字放大（text-lg 加粗），作为区块标题
- 移除番剧列表上方「番剧」小标题，布局更简洁
- 「+ 添加番剧」按钮移到筛选条件行右侧
- 评分/删除评分后立即刷新列表（接入 refreshKey 机制，无需手动刷新）
- 筛选条件新增「已评论/未评论」选项（后端 reviewed 参数按评论非空过滤）
- 筛选区重构：已评分/未评分 + 已评论/未评论整合为「我的状态」分组下拉（均针对当前登录用户）；排序补充「最早添加」；「+ 添加番剧」按钮与筛选控件同高（h-9）
- 评分/评论后首屏精选区与滚动卡片同步刷新（HeroBrand 与 HomePage 精选数据接入 refreshKey）
- 筛选「我的状态」新增「已收藏/未收藏」选项（后端 favorited 参数按收藏状态 want 过滤，与已评分/已评论同模式）

### 评分与评论
- 评分/评论解耦：支持「只打分不评论」和「只评论不打分」（score=0 + 评论）；只评论不打分不计入均分与评分人数（后端 avg/count 均过滤 score>0）
...
- 数据导入：新增 `scripts/import_excel.py`，可将人工维护的「极乐净土.xlsx」导入（番剧/游戏/工具 204 条 + 4 位朋友用户 324 条评分评论，幂等可重跑）
- Bangumi 匹配：`scripts/bangumi_match*.py` 系列脚本，为手动导入内容匹配 Bangumi 封面/简介/年份（直连回退代理）；匹配不上的不硬匹配（恢复占位图）
